...
<div class="justify-content-center">
    <div class="card">
        <div class="card-header text-center text-uppercase">{{ __('Update project') }}</div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ url('/project') }}/{{ $project->id }}">
                ...
                <div class="col-md-8 offset-md-2">
                    ...
                    <textarea id="description" class="form-control @error('description') is-invalid @enderror" name="description" rows="5">{{ $project->description }}</textarea>

                    <div class="row">
                        <div class="col-md-4">
                            <label for=startdatee" class="col-form-label text-md-right">{{ __('Start date') }}</label><br>
                            <input id="startdate" type="text" class="form-control date @error('startdate') is-invalid @enderror" name="startdate" value="{{ $project->startdate }}" required autocomplete="startdate" autofocus>
                        </div>
                        <div class="col-md-4">
                            <label for="deadline" class="col-form-label text-md-right">{{ __('Deadline') }}</label><br>
                            <input id="deadline" type="deadline" class="form-control date @error('deadline') is-invalid @enderror" name="deadline" value="{{ $project->deadline }}" required autocomplete="deadline" autofocus>
                        </div>
                        <div class="col-md-4">
                            <label for="status" class="col-form-label text-md-right">{{ __('Status') }}</label>
                            <select id="status" name="status" class="form-control @error('status') is-invalid @enderror">
                                <option value="Not started yet" {{ $project->status == 'Not started yet' ? 'selected' : '' }}>{{ __('Not started yet') }}</option>
                                <option value="In progress" {{ $project->status == 'In progress' ? 'selected' : '' }}>{{ __('In progress') }}</option>
                                <option value="Done" {{ $project->status == 'Done' ? 'selected' : '' }}>{{ __('Done') }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <select id="members" name="members[]" class="form-control @error('members') is-invalid @enderror" multiple>
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" @if(in_array($user->id, $project->members)) selected @endif>{{ $user->name }} {{ $user->lastname }}</option>
                        @endforeach
                    </select>
